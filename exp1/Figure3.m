...
% plot in kdensity in kHZ
% [f,xi] = ksdensity(var./1000, 'Support', 'positive'); 
% plot(xi, f, '-k', 'LineWidth', 2); hold on;
% 
% [f,xi] = ksdensity([lte_var gt_var]./1000, 'Support', 'positive'); 
...
%         ax = subplot(numel, 4, subplt);
%         pdvow = Dvow;
%         pdvow.(SID).resp(:, :, Dvow.meanf0<170) = NaN;
%         bins = [13 13];
%         plotImagescResponse(pdvow, SID, el,  1, ax, getColors(1), bins);    
%         
%         subplt = subplt+1;
%         ax = subplot(numel, 4, subplt);
%         pdvow = Dvow;
...
ax = subplot(1, 1, 1);
%sz = max([infl_f1.('sr_gt170') infl_f1.('sr_gt170')], [], 2, 'omitnan');
%sz = arrayfun(@(x) szmap(x, 200), sz);
z = infl_f1.el; % infl_f1.SID; %infl_f1.el; % 
...
function [inflections] = loadF0Infl(formant, infoStruct, SIDs)
    ...
    for s = 1:length(SIDs)
        ...
        if isfield(infoStruct{1}, SID) && ~isempty(infoStruct{1}.(SID).els)   
            ...
            sids = repmat(str2double(SID(3:end)), length(infoStruct{1}.(SID).els), 1);
