...
%     plot(x(1:end)./1000, smoothdata(median(cl), 'SmoothingFactor',0.1), ...
%         'Color', cols(v, :), 'LineWidth', 1); hold on;
...
Dvow = addtoDD(Dvow, 'dimex', bef, aft, {'EC100', 'EC214'});
cols = getColors(1);

% E1
erpSIDs = {'EC100', 'EC100', 'EC100'}; 
els = [55 71, 118];
...
elecs('EC100') = [55 71 118]; % E1 and E2, ERP electrode  118
elecs('EC214') = 54; % E3, Sigmoid 54
...
load([datapath 'pt_data/out_elecs_speechtypeftest_bychan_dimex_Spanish.mat'], ...
    'allidx', 'fvals');
allidx_speech = allidx;
fvals_speech = fvals;

load('pt_data/out_elecs_voweltypeftest_bychan.mat', 'allidx', 'fvals');
allidx_vowel = allidx;
fvals_vowel = fvals;
...
[~] = plotNativeElec({'EC100'}, desel, [datapath '/pt_data']);
% print(fullfile('EC100_vowel.jpg'), '-djpeg', '-painters', '-r600')
set(gcf,'Color','w');
...
elecs = containers.Map;
elecs('EC100') = []; % E1 - E3, ERP electrodes 55 71 118
% elecs('EC214') = 54; % E3, Sigmoid
...
[stgelecs,~] = getElecs(Dvow, SIDs, 'dimex', 'anatomy', struct(), ...
    [datapath '/pt_data']);
load('out_elecs_voweltypeftest_bychan.mat', 'fvals', 'allidx');
...
for s = SIDs    
    ...
    % determining number of non-nan trials
%     trials = squeeze(mean(Dvow.(SID).resp, [1, 2], 'omitnan'));    
%     disp(['trials presented to ' SID ': ' num2str(sum(~isnan(trials)))])  
end
...
%% Figure 1 - SUPP: Proportion of speech responsive STG to vowel responsive

typeinfo = struct();
typeinfo.resp = 'resp';
typeinfo.modelname = {'onset_maxDtL_maxDtLOnset_vowelOnset_aud'};
[~, rsqs] = getElecs(Dvow, SIDs, 'dimex', 'strfrsq', typeinfo, [datapath '/pt_data']);
[stgelecs,~] = getElecs(Dvow, SIDs, 'dimex', 'anatomy', struct(), [datapath '/pt_data']);
load('out_elecs_voweltypeftest_bychan.mat', 'fvals');

figure('Position', [100 800 300 300]); 
for s = SIDs
    SID = s{1};
    if isfield(rsqs, SID)
        %cols = [0.5 0.8 0.6; 0.8 0.15 0.95];
        stg = ismember(1:length(fvals.(SID)), stgelecs.(SID));
        scatter(fvals.(SID)(stg), rsqs.(SID)(stg), 45, [0.8 0.8 0.8], 'filled', ...
            'MarkerFaceAlpha', 0.7, 'MarkerEdgeColor', 'k', 'LineWidth', 0.025); hold on;
        scatter(fvals.(SID)(~stg), rsqs.(SID)(~stg), 35, [0.2 0.2 0.2], 'filled', ...
...
filename = ['EC105_B1_HilbAA_70to150_8band_all_0_mel_DIMEX_' ...
    'zflag_global_out_resp_log.mat'];
load([datapath '/pt_data/EC105/dimex/' filename])
...
corpusStrf=loadMultModelStrf('EC105', modelnames, corpus, ...
    [datapath '/pt_data'], 1, '');
...
load([datapath '/pt_data/out_elecs_voweltypeftest_bychan.mat'], ...
    'allidx', 'fvals');
for s = SIDs
    SID = s{1};    
