...
for s = SIDs
    ...
    if isfield(betaInfo.(SID), 'modelrsq')
        ...
        subj(ctr:ctr+size(idx, 1)-1) = repmat(str2double(SID(3:end)), 1, size(idx, 1));
...
%     [x_cross,~] = polyxpoly(x_n,y_n,x_p,y_p);
%     err = (sum(pos_slope<x_cross) + sum(neg_slope>x_cross)) ...
%         / length([pos_slope neg_slope]);
%     disp(['F' num2str(f) ' Perc error: ' num2str(err)]);
%     [~, p, ci, stats] = ttest2(pos_slope, neg_slope);
%     disp(['F' num2str(f) ' p=' num2str(p) ', tstat=' num2str(stats.tstat) ...
%         ', df=' num2str(stats.df)]);
...
for s = 1:length(SIDs)
    ...
    if isfield(betaInfo, SID) && ~isempty(betaInfo.(SID).els)        
        ...
        showSID = 1;
        if showSID
            z = repmat(str2double(SID(3:end)), 1, length(idx));
        else
...
for f = formants
    ...
    for s=1:length(SIDs)
        ...
        tblidx = inflections.SID==str2double(SID(3:end));
...
%% Figure 2 - SUPP: Calculating BIC for all fits

bic = nan(2, height(inflections), 2);
aic = nan(2, height(inflections), 2);
for f = 1:2
    for row = 1:height(inflections)
        SID = ['EC' num2str(inflections{row, 'SID'})];
        el = inflections{row, 'el'};
        elidx = find(betaInfo.(SID).els == el);
        x = betaInfo.x(f, :);
        y = squeeze(betaInfo.(SID).y(elidx, f, :))';

        % calculate linear predictions and log-likelihood   
        param = betaInfo.(SID).beta_lin(elidx, f, :);
        y_pred = param(1)*x + param(2);
        ll(1) = log(mean((y-y_pred).^2, 'omitnan'));

        % calculate sigmoidal predictions and log-likelihood
        param = squeeze(betaInfo.(SID).beta_sigm(elidx, f, :));
        fsigm = @(param,xval) ...
                           param(1)+(param(2)-param(1))./...
                           (1+10.^((param(3)-xval)*param(4)));
        y_pred = fsigm(param, x);
        ll(2) = log(mean((y-y_pred).^2, 'omitnan'));

        % calculate bayesian information criterion
        [aic(f, row, :), bic(f, row, :)] = aicbic(ll, [2, 4]);
    end
end
inflections.bic_f1 = squeeze(bic(1, :, :));
inflections.bic_f2 = squeeze(bic(2, :, :));

inflections.aic_f1 = squeeze(aic(1, :, :));
inflections.aic_f2 = squeeze(aic(2, :, :));

% figure v1
for f = 1:2
    subplot(1, 2, f);
    sr = inflections.(['sr_f' num2str(f)]);
    lr = inflections.(['lr_f' num2str(f)]);
    slope =  inflections.(['sl_f' num2str(f)]);
    
    % red for negative, blue for positive
    RB = [1 0 0; 0 0 1]; 
    colors = cell2mat(arrayfun(@(x) RB(x+1, :), slope, ...
        'UniformOutput', false));                

    bic_f = inflections.(['bic_f' num2str(f)]);
    sig_pos = (bic_f(:, 1)>bic_f(:, 2));
    
    scatter(lr(~sig_pos), sr(~sig_pos), 25, colors(~sig_pos, :), 'filled', ...
            'MarkerFaceAlpha', 0.5, ...
            'HandleVisibility', 'off'); hold on;    
...
% figure v2
figure('Position', [800, 400, 800, 300]);
...
for f = 1:2
    ...
    idx = bic_f(:, 1)>bic_f(:, 2);
    scatter(bic_f(:, 1), bic_f(:, 2), 65, ...
        double(idx), 'filled', 'MarkerFaceAlpha', 0.5);
    
    disp(['F' num2str(f) ' Sigmoidal over linear: ' ...
        num2str(sum(idx)) '/' num2str(height(inflections)) ...
        ', ' num2str((sum(idx)/height(inflections))*100)])
    
    colormap([0.5 0.5 0.5; 0.1 0.6 0.3]);
    ...
    title(['Formant ' num2str(f)]);
    xlabel('linear BIC score'); 
    ylabel('sigmoidal BIC score');
...
figure;
load('out_elecs_voweltypeftest_bychan.mat', 'allidx', 'fvals');
...
for s = SIDs
    ...
    if ~isempty(corpusStrf{1})
        ...
        for el = find(rsq_aud > 0.15) 
            tidx = find(inflections.SID == str2double(SID(3:end)) ...
                    & inflections.el == el);
...
%% Figure 2 - SUPP: Latency of Sigmoidal vs. Linear Fit Over All Electrodes
...
for f = 1:2 
    for s = SIDs
        ...
        corpStrf = loadMultModelStrf(SID, modelnames, 'dimex', ...
            [datapath '/pt_data'], 1, '');
        corpStrf = corpStrf{1};
        ...
        subtable = inflections(inflections.SID == str2double(SID(3:end)), :);

        sr = subtable.(['sr_' column{f}]);
        lr = subtable.(['lr_' column{f}]);      
        elecs = subtable(sr > 0, 'el');
        latency = maxidx(elecs.el);
        sigm_diff = sr(sr>0) - lr(sr>0);
        
        debug = 0;
        if debug
            figure;
            imagesc(squeeze(...
                mean(corpStrf.meanStrf(strtfeat:end, :, elecs.el), 'omitnan'))');
            hold on;
            ctr = 1;
            for el = elecs.el'
                plot([maxidx(el), maxidx(el)], [ctr-0.5, ctr+0.5], '-r', ...
                    'LineWidth', 2);
                ctr = ctr+1;
            end           
        end
        ...
        subplot(1, 2, f);
        scatter(sigm_diff, latency*0.01, 45, sr(sr>0), 'filled', ...
...
for i =1:2
    ...
    xlabel('Sigm R^2 - Lin R^2');    
    if i == 1
        ylabel('Latency of Peak Response');
    end
    set(gca, 'FontSize', 15);
